...
export default class spservices {
  ...
  public async getTaskDetails(taskId: string): Promise<ITaskDetails> {
    try {
      const graph = graphfi().using(graphSPFx(this.context));
      const taskDetails: ITaskDetails = await graph.planner.tasks.getById(taskId).details();
      return taskDetails;
    } catch (error) {
      Promise.reject(error);
    }
...
planId:string :Promise<string>  */
  public async getPlan(planId: string): Promise<IPlannerPlan> {
    try {
      ...
      const plannerPlan: IPlannerPlan = await graph.planner.plans.getById(planId)();
  ...
  public async getPlanDetails(planId: string): Promise<IPlannerPlanDetails> {
    try {
      ...
      const plannerPlanDetails: IPlannerPlanDetails = await graph.planner.plans.getById(planId).details();
